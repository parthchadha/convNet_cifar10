...
def build_model_4_2(input_var=None):
	...
	conv_1 = lasagne.layers.Conv2DLayer(
			drop_0, num_filters=32, filter_size=(8, 8),
...
def build_model_3_2(input_var=None):
	...
	# convolutions are supported as well; see the docstring.
	conv_1 = lasagne.layers.Conv2DLayer(
			drop_0, num_filters=64, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu'))
	# Expert note: Lasagne provides alternative convolutional layers that
	# override Theano's choice of which implementation to use; for details
...
#	network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_2 = lasagne.layers.Conv2DLayer(
			drop_1, num_filters=64, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
	#network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_3 = lasagne.layers.Conv2DLayer(
			drop_2, num_filters=128, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
...
def build_model_2_2(input_var=None):
	...
	# convolutions are supported as well; see the docstring.
	conv_1 = lasagne.layers.Conv2DLayer(
			drop_0, num_filters=96, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu'))
	# Expert note: Lasagne provides alternative convolutional layers that
	# override Theano's choice of which implementation to use; for details
...
#	network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_2 = lasagne.layers.Conv2DLayer(
			drop_1, num_filters=192, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
	#network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_3 = lasagne.layers.Conv2DLayer(
			drop_2, num_filters=160, filter_size=(3, 3),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
...
#	network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_2 = lasagne.layers.Conv2DLayer(
			drop_1, num_filters=96, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
	#network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
	...
	# Another convolution with 32 5x5 kernels, and another 2x2 pooling:
	conv_3 = lasagne.layers.Conv2DLayer(
			drop_2, num_filters=160, filter_size=(5, 5),
			nonlinearity=lasagne.nonlinearities.rectify,
			W=lasagne.init.GlorotUniform('relu')
			)
...
def main(x = 1, save=1, num_epochs=100,alpha=0.001,mv=0.9,l2_rate = 0.001):
	...
	file_name = 'model_' + str(x)+ '_2_alpha_' + str(alpha) +'_momentum_' + str(mv) + '_l2_reg_' + str(l2_rate)+'_with_dropout.npz'
	...
	data_file_name = 'data_value_all_model_' +str(x) +'_2_alpha_'+ str(alpha) +'_momentum_' + str(mv) + '_l2_reg_' + str(l2_rate)+'_with_dropout.pckl'
